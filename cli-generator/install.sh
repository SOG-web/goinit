...
get_latest_version() {
    ...
    LATEST_VERSION=$(curl -s https://api.github.com/repos/rou/goinit/releases/latest | grep '"tag_name"' | sed -E 's/.*"([^"]+)".*/\1/')
...
install_binary() {
    ...
    DOWNLOAD_URL="https://github.com/rou/goinit/releases/download/${LATEST_VERSION}/${BINARY_NAME}.tar.gz"
    if [ "$OS" = "windows" ]; then
        DOWNLOAD_URL="https://github.com/rou/goinit/releases/download/${LATEST_VERSION}/${BINARY_NAME}.zip"
    fi
